...
for (i in 1:length(deconv_params)) {
  if (deconv_params[i] == TRUE) {
    ...
    colnames(deconv_res) <- paste(gsub(" ","_",colnames(deconv_res)),deconv_method,"ImmDeconv",sep = "_")
...
####----Bin Scores by Median Cut-point----####

deconv_cols <- colnames(deconv_res_df)[c(2:ncol(deconv_res_df))]
decon_bin_cols <- c()
for (i in deconv_cols) {
  new_colname <- paste(i,"_HiLoScore",sep = "")
  deconv_res_df[,new_colname] <- highlow(deconv_res_df[,which(colnames(deconv_res_df) == i)])
  decon_bin_cols <- c(decon_bin_cols,new_colname)
}

## subset out scores cols to write file
deconv_res_scores_df <- deconv_res_df[,c("SampleName",deconv_cols)]
...
write_delim(deconv_res_scores_df,paste(Output_Path,ProjectName,"_ImmuneDeconvolution.txt", sep = ""),delim = '\t')
